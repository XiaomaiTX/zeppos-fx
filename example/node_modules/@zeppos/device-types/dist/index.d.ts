...
declare namespace DataWidget {
    interface Option {
        /**
         * @zh DataWidget 实例上挂载的数据对象，可用于存储状态
         * @en A data object mounted on a DataWidget instance that can be used to store the state of the current DataWidget
         */
        state?: object;
        /**
         * @zh 初始化完成时触发，只触发一次，可以用来初始化应用插件状态
         * @en It is triggered once per DataWidget and can be used to initialize the DataWidget state
         */
        onInit?: (params?: string) => void;
        /**
         * @zh 在 `onInit` 执行完成后触发，推荐在 `build` 生命周期中进行 UI 绘制
        ...
        build?: (params?: string) => void;
        /**
         * @zh 当屏幕焦点聚焦在此应用插件上时触发
         * @en Triggered when the screen focus is on this DataWidget
         */
        onResume?: () => void;
        /**
         * @zh 当屏幕焦点离开此应用插件上时触发
         * @en Triggered when the screen focus leaves this DataWidget
         */
        onPause?: () => void;
        /**
         * @zh 销毁时触发 `onDestroy` 生命周期函数
         * @en The `onDestroy` lifecycle function is triggered when the DataWidget is destroyed
         */
        onDestroy?: () => void;
    }
    ...
    /**
     * @zh DataWidget 实例
     * @en DataWidget instance
     */
    type Result = object;
}

/**
 * @zh 注册应用插件，指定当前页面的生命周期回调等。每个应用插件都必须调用 `DataWidget()` 构造函数且只能调用一次
 * @en Register DataWidget, specify the lifecycle callback for the current DataWidget, etc. Each DataWidget file must call the `DataWidget()` constructor only once
 * @version 3.6
 * @example
 ...
 * ```
 */
declare function DataWidget(option: DataWidget.Option): DataWidget.Result;
 ...
 * @en Register an App Service in the Mini Program, specify the lifecycle callback for the current App Service, etc. Each App Service file must call the `AppService()` constructor only once
 * @permissionCode device:os.bg_service
 * @version 3.0
 * @example
    ...
    namespace DeviceSide {
        namespace HmUI {
            ...
            interface IHmUIWidget {
                ...
                getProperties: (...names: string[]) => Record<string, any>;
                visible: boolean;
                x: number;
                y: number;
                w: number;
            ...
            interface IHmUIExtensionFunction {
                 ...
                 */
                setScrollView(enable: boolean, pageHeight?: number, pageCount?: number, isVertical?: boolean, opts?: any): boolean;
                /**
                 * @zh 设置当前页面是否可以滑动
                 * @en Set whether the current page can be slid
                 */
                setLayerScrolling(enable: boolean, opts?: any): boolean;
                /**
